    ...
    <div className="mx-auto max-w-[1380px]">
      <section className="fade-up flex flex-col justify-between gap-6 border-b border-[#cfd2c9] pb-7 md:flex-row md:items-end">
        <div>
         ...
         <div className={`flex shrink-0 items-center gap-3 border-l-2 ${fieldRun.decision === 'go' ? 'border-[#236047]' : 'border-[#b95042]'} bg-[#fbfbf7] px-4 py-3`}>
      ...
      <section className="fade-up fade-up-2 grid gap-5 xl:grid-cols-[1.4fr_0.85fr]">
        <div className="border border-[#d7d8d0] bg-[#fbfbf7]">
          <div className="flex flex-col gap-4 border-b border-[#d7d8d0] px-5 py-5 sm:flex-row sm:items-start sm:justify-between">
             <div><SectionKicker>Readiness summary</SectionKicker><h2 className="mt-1 font-display text-xl font-extrabold tracking-[-0.04em] text-[#203c49]">{fieldRun.decision === 'go' ? 'All physical gates passed.' : 'The handoff is NO-GO.'}</h2><p className="mt-1 max-w-xl text-sm leading-5 text-[#687271]">{measured === 0 ? 'No physical Pixel observations have been recorded. The sample records below are not a production-readiness claim.' : `${measured} of ${gates.length} physical observations are recorded. Missing, failed, or inconclusive gates keep the handoff blocked.`}</p></div>
        ...
        <div className="border border-[#d7d8d0] bg-[#203c49] text-[#f2f0e6]">
          ...
          <div className="space-y-0 px-5">
            <div className="flex gap-4 border-b border-[#3a5962] py-4"><Smartphone size={17} className="mt-0.5 shrink-0 text-[#ffd067]" /><div><p className="text-xs font-bold">Managed Google Pixel 11</p><p className="mt-1 text-xs leading-5 text-[#aec0b8]">Approved physical target · stock Android · actual build recorded at preflight</p></div></div>
             <div className="flex gap-4 border-b border-[#3a5962] py-4"><MapPin size={17} className="mt-0.5 shrink-0 text-[#ffd067]" /><div><p className="text-xs font-bold">{fieldRun.observations.location ? 'Location measured on device' : 'Location measurement pending'}</p><p className="mt-1 text-xs leading-5 text-[#aec0b8]">{fieldRun.observations.location ? fieldRun.observations.location.notes || 'Recorded physical observation.' : 'The 18 m sample accuracy is not physical validation.'}</p></div></div>
